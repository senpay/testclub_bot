...
class UserRepositoryDis(Repository):
    ...
    def increase_badmessages(self, user_name):
        user=self.get_user_by_id(user_name)
        logging.info(f'Badmessages: {user_name}')
        user['badmessage']=user['badmessage']+1
        self._database.put(user)
        return

    def kol_badmessages(self,user_name):
        user=self.get_user_by_id(user_name)
        ret=user['badmessage']
        self._database.put(user)
        return ret
...
class Confmesss(Repository):

    def __init__(self):
        self._kind = 'Confmess'
        super(Confmesss, self).__init__()

    def add_post(self,message, typemess):
        post = self._get_entity()
        post['message'] = message
        post['typemess'] = typemess
        self._database.put(post)

    def get_commands(self):
        logging.info(f'Getting commands')
        return self._get_records()