import logging
from attr import dataclass

from repositories import Confmess


Confmessages=Confmess()
logger = logging.getLogger(__name__)


@dataclass
class Member:
    name: str


@dataclass
class Message:
    author_name: str
    text: str
    channel_name: str


class MessageSender:
    def send_to_general(self, message: str):
        raise NotImplementedError()

    def send_to_channel_name(self, channel_name: str, message: str):
        raise NotImplementedError()


class MembersService:

    def __init__(self, bot_name: str, message_sender: MessageSender):
        self.bot_name = bot_name
        self.message_sender = message_sender


    def handle_member_left(self, member: Member):
        logger.info(f'{member.name} left our server. Bastard!!1')
        ps=Confmess.get_commands()
        for i in range(0,len(ps)):
            if ps[i]['typemess']=='ON_MEMBER_LEFT_MSG':
                ON_MEMBER_LEFT_MSG=ps[i]['typemess']
        self.message_sender.send_to_general(ON_MEMBER_LEFT_MSG.format(member.name))

    def handle_message(self, msg: Message):
        ps=Confmess.get_commands()
        for i in range(0,len(ps)):
            if ps[i]['typemess']=='PING_COMMAND':
                PING_COMMAND=ps[i]['typemess']
            if ps[i]['typemess']=='PONG_MSG':
                PONG_MSG=ps[i]['typemess']
        if msg.author_name != self.bot_name and msg.text == PING_COMMAND:
            self.message_sender.send_to_channel_name(msg.channel_name, PONG_MSG)

    def handle_member_joined(self, member: Member) -> str:
        logger.info(f'{member.name} joined our server. Good!!1')
        ps=Confmess.get_commands()
        for i in range(0,len(ps)):
            if ps[i]['typemess']=='ON_MEMBER_JOINED_MSG':
                ON_MEMBER_JOINED_MSG=ps[i]['typemess']
        self.message_sender.send_to_general(ON_MEMBER_JOINED_MSG.format(member.name))










